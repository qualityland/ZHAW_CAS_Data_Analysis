...
## Abstand zwischen zwei Knoten h?tte  2 cm entsprechen sollen, aber ....
##
...
## mitgez?hlt haben, die anderen nicht.
...
## Die Messungen zeigen ?ber die Zeit einen Trend abw?rts.
## Das Messsystem war nicht stabil. Durch das h?ufige strecken
...
## es nicht klar ist, was gemessen werden soll. Das Holzst?bchen ist
...
##  Werte zwischen 12.7 und 13.0 sind "m?glich")

## Durch Wiederholen der Messungen k?nnen wird die Genauigkeit der Messung
## erh?hen.
...
boxplot(EM9, las=1, col="blue", horizontal=TRUE,
        xlab="L?nge in 'Knoten'",  ylab="")
hist(EM9, col="blue", las=1, nclass=12)
## Verteilung unimodal, rechtsschief (mit Ausreisser?)
stripchart(EM9, method="stack", pch=4)
## Gem?ss eigenen Aussagen haben nicht wenige den Startknoten mitgez?hlt.
scatter.smooth(EM9, type="b", col="blue")
## einmal erster Knoten mitgez?hlt (Beob.9); ab Beobachtung 5 Trend nach unten,
## zuvor ansteigender Trend
##
mean(EM9)     ## = 6.244643 Knoten
## Bei drei Messungen wurde der erste Knoten mitgez?hlt, bei den ersten beiden
## Messungen wurde der "Nullpunkt" am Anfanf der Schnur gelegt.
## Folglich waren die Anweisungen, wie gemessen werden soll ungen?gend.
...
## (b)
## Gesucht ist P(0.3 - 0.005 <= X <= 0.3 + 0.005) f?r X~ N(0.302, sigma=0.003),
## also:
...
## Betrachten die standardisierte ZV Z:= (X-mu)/sigma, dann gilt f?r Z:
## P(Z <= (400-mu)/4) = 0.02
...
## Eine Verringerung der Standardabweichung um 1.5g f?hrt also zu einer
## Verringerung der n?tigen durchschnittlichen F?llmenge um etwas mehr als 3g!
...
n <- 500             ## L?nge des Vektors aus dem der Mittelwert berechnet wird
...
## ausser dass der Wertebereich auf der x-Achse viel, viel gr?sser ist als f?r
...
## Bei brust k?nnen sowohl die Orginal- wie auch die logarithmierten Daten
## durch eine Normalverteilung beschrieben werden, da in beiden F?llen die
...
## Die gleiche Feststellung gilt f?r die waerme-Daten.
