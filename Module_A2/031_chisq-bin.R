...
chisq.bin <- function(x, f, n = NULL, p = NULL, estimated=TRUE) {
    ...
    chisq <- (obs - esp)^2 / esp
    chisq.t <- sum(chisq)
    if(estimated) dft <- length(chisq) - k - 1 - 1  ## Korrektur ARu
    else dft <- length(chisq) - k - 1
    ...
    dist.info <- paste("Chi-square goodness of fit test for a binomial(",
                       n, ", ", format(p, digits = 4), ") distribution",
                       sep = "")
